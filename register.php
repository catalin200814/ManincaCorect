...
<!DOCTYPE html>
<html lang="ro">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>PlanMeal – Înregistrare</title><link rel="stylesheet" href="css/style.css"><link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700&family=Inter:wght@400;500;600&display=swap" rel="stylesheet"></head>
<body>
<nav class="navbar"><div class="navbar-inner"><a href="index.php" class="logo"><span class="logo-icon">🍽️</span><span class="logo-text">PlanMeal</span></a><div class="nav-cta"><button id="theme-toggle" class="theme-switch">🌓</button><a href="login.php" class="btn-ghost">Conectare</a></div></div></nav>
<div class="auth-page"><div class="auth-card"><div class="auth-header"><h2>Crează-ți cont</h2><p>Înregistrează-te gratuit pe PlanMeal</p></div><?php if ($error): ?><div class="alert alert-error"><?= htmlspecialchars($error) ?></div><?php endif; ?><?php if ($success): ?><div class="alert alert-success"><?= htmlspecialchars($success) ?> <a href="login.php">Conectează-te aici</a></div><?php endif; ?>
<form method="POST" class="auth-form"><div class="form-group"><label>Nume complet</label><input type="text" name="nume" required></div><div class="form-group"><label>Email</label><input type="email" name="email" required></div><div class="form-group"><label>Parolă (min 6)</label><input type="password" name="parola" required></div><div class="form-group"><label>Confirmă parola</label><input type="password" name="confirma" required></div><button type="submit" class="btn-primary btn-full">Înregistrează-te</button></form><div class="auth-footer">Ai deja cont? <a href="login.php">Conectează-te</a></div></div></div>
<footer class="footer">
